...
void test_messages(dd_demo_writer *dw, int tick) {
    assert(demo_w_write_msg_sv_motd(dw, tick, "Welcome!"));
    assert(demo_w_write_msg_sv_broadcast(dw, tick, "Broadcast message"));
    assert(demo_w_write_msg_sv_chat(dw, tick, 1, 0, "Chat message"));
    assert(demo_w_write_msg_sv_killmsg(dw, tick, 0, 1, DD_WEAPON_HAMMER, 0));
    assert(demo_w_write_msg_sv_sound_global(dw, tick, DD_SOUND_CTF_CAPTURE));
    assert(demo_w_write_msg_sv_weaponpickup(dw, tick, DD_WEAPON_SHOTGUN));
    assert(demo_w_write_msg_sv_emoticon(dw, tick, 0, DD_EMOTE_HAPPY));
    
    const char *options[15] = {0};
    options[0] = "Opt1";
    options[1] = "Opt2";
    assert(demo_w_write_msg_sv_vote_option_list_add(dw, tick, 2, options));
    
    assert(demo_w_write_msg_sv_vote_option_add(dw, tick, "Opt3"));
    assert(demo_w_write_msg_sv_vote_option_remove(dw, tick, "Opt1"));
    assert(demo_w_write_msg_sv_vote_set(dw, tick, 30, "Vote Desc", "Reason"));
    assert(demo_w_write_msg_sv_vote_status(dw, tick, 5, 2, 0, 7));
    assert(demo_w_write_msg_sv_ddrace_time_legacy(dw, tick, 1000, 1, 0));
    assert(demo_w_write_msg_sv_record_legacy(dw, tick, 900, 950));
}
...
void test_complex_deltas() {
    ...
    while(demo_r_next_chunk(dr, &chunk)) {
        if (chunk.type == DD_CHUNK_SNAP_DELTA && chunk.tick == 11) {
            int u_size = demo_r_unpack_delta(dr, chunk.data, chunk.size, unpacked);
            assert(u_size > 0);
...
void test_full_workflow() {
    ...
    // We wrote 14 messages in test_messages
    assert(msg_count == 14);
